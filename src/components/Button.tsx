...
export function Button({ className, variant = 'text', ...rest }: Props) {
  ...
  const containedStyles = 'bg-blue-700 text-white shadow-sm hover:bg-blue-800 hover:shadow-md' // contained styles
  ...
  return (
    <button
      type="button"
      className={clsx({
        'text-md relative mb-2 inline-flex cursor-pointer items-center justify-center rounded-md px-8 py-2 font-semibold uppercase leading-6 tracking-wide': true,
        transition: true,
...
export function DeleteButton({ className, ...rest }: Props) {
  return (
    <Button className={clsx('text-center text-sm font-medium uppercase text-red-700 underline', className)} {...rest} />
  )
}
